...
import json
import copy
regex_pattern = r'[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+\w[А-Я+Ѣ+І]|[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+ [А-Я+Ѣ+І] |\
                  [А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+|[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+|[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+ [А-Я+Ѣ+І]'
...
def identify_character_names(line):
    ...
    pattern = r'[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+|\
                [А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+ [А-Я+Ѣ+І] |[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+|\
                [А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+|[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+'
...
def handle_alternative_names(line):
    ...
    Returns:
        character_name - how it appears in the dramatic character cast
        alternative names -  a list of other names under which the dramatic character appears in the text.
    """
...
def split_acts(play_text, number_acts, old_ortho_flag):
    """
    The function splits the text of the play into acts. If the number doesn't match expected number,
    raises an exception.
    Params:
...
def estimate_verse_line_splitting_stage_directions(text_string):
    ...
    for split in splits:
        ...
        line = remove_start_end_stage_directions(
                                                clean_stage_direction(replace_tags(remove_numbers(split))))
...
def verse_split_between_scenes(play_text, old_ortho_flag=True):
    ...
    for scene in scenes[1:]:
        ...
        if check_end_of_scene(scene_cleaned):
            pattern = r'ЯВЛЕНІЕ +\w+|ЯВЛЕНИЕ +\w+|[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+|[А-Я+Ѣ+І]+.\w[А-Я+Ѣ+І]+'
            entities = re.findall(pattern, scene)
...
def estimate_number_scenes(scene_summary):
    ...
    for key in scene_summary.keys():
        ...
        total_number_scenes_per_text += len([scene for scene in scene_summary[key].keys()
                                            if scene.count('extra') == 0])
        # count scenes as marked by actual entrances and exits
        total_number_scenes_iarkho += len([scene for scene in scene_summary[key].keys()
